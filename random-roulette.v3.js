/* Random Roulette modular build · 2026-08-25T01:17:05.769Z */
...
function shouldUseRiskFallbackForEmoji(emoji) {
  return EMOJI_RISKY_FALLBACK_SET.has(emoji) && isConservativeEmojiFallbackEnvironment() && !isEmojiLikelySupported(emoji)
}
...
function installEmojiFallbacks() {
  ...
  if (typeof requestIdleCallback === 'function') {
    requestIdleCallback(runInitialCheck, { timeout: APP_PERFORMANCE_PROFILE.constrained ? 2600 : 1200 })
  } else {
    setTimeout(runInitialCheck, APP_PERFORMANCE_PROFILE.constrained ? 900 : 250)
  }
}
...
function handleLuckGameSelection(button) {
  ...
  if (button.classList.contains('game-launch')) {
    ...
    if (selectedGame === '8') {
      const smartPick = window.RandomRouletteRegistry?.pickEligibleLuckScreen?.()
      if (!smartPick) return
...
(function installGameRegistry(global) {
  const games = Object.freeze({
    ...
    game7: { title: '투명 사다리', min: 2, max: () => isPhoneLikeDevice() ? 4 : 8, group: 'luck' },
  ...
  function isPhoneLikeDevice() {
    const coarseOnly = global.matchMedia?.('(pointer: coarse) and (hover: none)')?.matches
    return Boolean((coarseOnly || navigator.maxTouchPoints > 0) && Math.min(innerWidth, innerHeight) <= 820)
  }
  ...
  function getEligibility(screenKey, options = {}) {
    ...
    if (count < config.min) {
      return { ok: false, reason: `공용 참가자 명단에 최소 ${config.min}명을 먼저 등록해줘.` }
    }
    if (count > max) {
      return { ok: false, reason: `${config.title}은 현재 ${count}명 명단으로 실행할 수 없어. 지원 인원은 ${config.min}~${max}명이야.` }
    }
  ...
  function refreshCards() {
    document.querySelectorAll('#luckGameGrid .game-item').forEach((button) => {
      const game = button.dataset.game
      if (game === '8') {
        const hasEligible = getEligibleLuckScreens().length > 0
        ...
        if (hasEligible) {
          ...
          button.title = '공용 참가자 명단을 등록하면 실행 가능한 게임만 골라줘.'
        }
        return
      }
      markCard(button, luckButtonToScreen[game])
    })
    document.querySelectorAll('#physicalGameGrid .physical-game-item').forEach((button) => {
      markCard(button, physicalButtonToScreen[button.dataset.physicalGame])
    })
  }

  function getEligibleLuckScreens() {
    return Object.entries(games)
      .filter(([, config]) => config.group === 'luck')
      .map(([screen]) => screen)
      .filter((screen) => getEligibility(screen, { forSmartPick: true }).ok)
  }

  function pickEligibleLuckScreen() {
    if (!global.RandomRouletteRoster?.hasRoster?.()) {
      global.RandomRouletteRoster?.open?.()
      return null
    }

    const eligible = getEligibleLuckScreens()
    if (!eligible.length) {
      showPopup('실행 가능한 게임 없음', '현재 인원수로 실행 가능한 운 게임이 없어. 공용 명단의 인원을 조정해줘.', { icon: '⚠️' })
      return null
    }
  ...
  global.RandomRouletteRegistry = Object.freeze({
    ...
    games,
    getEligibility,
    getEligibleLuckScreens,
    pickEligibleLuckScreen,
    refreshCards
  })
})(window)
