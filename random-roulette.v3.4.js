/* Random Roulette modular build · 2026-08-25T03:11:08.913Z */
...
(function installGameRegistry(global) {
  ...
  function isPhoneLikeDevice() {
    const supportsPointerQueries = typeof global.matchMedia === 'function'
    const coarsePrimary = global.matchMedia?.('(pointer: coarse)')?.matches
    const noHover = global.matchMedia?.('(hover: none)')?.matches
    const touchFallback = !supportsPointerQueries && navigator.maxTouchPoints > 0
    return Boolean(((coarsePrimary && noHover) || touchFallback) && Math.min(innerWidth, innerHeight) <= 820)
  }
